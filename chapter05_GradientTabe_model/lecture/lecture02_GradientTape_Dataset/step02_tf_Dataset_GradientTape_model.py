...
x_train.shape # (60000, 28, 28) : (size, h, w) -2d
y_train.shape # (60000,) 
...
# 3. model 정의 : 순방향 (input @ w + b -> y_pred)
...
def loss_fn(X, y): # 입력, 정답 
    ...
    y_pred = model.call(X) # model.call(X) : 예측치  
    loss = -tf.reduce_mean(y*tf.math.log(y_pred) + (1-y)*tf.math.log(1-y_pred)) 
    return loss # 손실 반환 



# 7. 미분계수 자동 계산 : 역방향(loss -> 기울기 -> [w, b] update)
...
def model_train(X, y): 
    ...
    with tf.GradientTape() as tape : # 매번 new object 생성    
        ...
        loss = loss_fn(X, y) 
        # 2) 역방향 : 접선의 기울기 계산 
        # model.trainable_variables -> 조절변수[w, b]
...
# 8. model 학습 
for epoch in range(10): # epochs = 10 : 60000 * 10 = 600,000
    # 초기화
